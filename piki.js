// Old browsers
if (!window.console)console={log:function(content){alert('Log: '+content);},info:function(content){alert('Info: '+content);}};
...
(function($){
	...
	$(function(){
		...
		$( window ).bind( 'resize', function(){
			...
			_resize_timeout = setTimeout( window.bindResize, 100 );
		});
		window.bindResize();

	});

	window.bindResize = function(){
		
		// Gadget atual pelo tamanho
		var gadget = window.gadgetBySize();
		
		// Class atual do body
		var before = false;
		if( _body.is( '.is-phone' ) ){
			before = 'phone';
		}
		else if( _body.is( '.is-tablet' ) ){
			before = 'tablet'
		}
		else if( _body.is( '.is-desktop' ) ){
			before = 'desktop';
		}
		...
		if( gadget === 'phone' && before !== 'phone' ) {
			_body.removeClass( 'is-desktop is-tablet' );
			_body.addClass( 'is-mobile is-phone' );
			reconfigureImages = true;
		} 
		else if( gadget === 'tablet' && before !== 'tablet' ) {
			_body.removeClass( 'is-desktop is-phone' );
			_body.addClass( 'is-mobile is-tablet' );
			reconfigureImages = true;
		} 
		else if( gadget === 'desktop' && before !== 'desktop') {
			_body.removeClass( 'is-tablet is-phone is-mobile' );
		...
		if( reconfigureImages ){
			$( 'img._responsive-srcs' ).each(function(){
				window.bindResizeImage.call( this, gadget );
			});
		}
		
	};

	window.bindResizeImage = function( gadget ){

		// Phone
		if( gadget === 'phone' ){
			if( this.phone_src !== undefined ){
				this.$.attr( 'src', this.phone_src );
				return;
			}
			else if( this.mobile_src !== undefined ){
				this.$.attr( 'src', this.mobile_src );
				return;
			}
		}
		// Tablet
		else if( gadget === 'tablet' ){
			if( this.tablet_src !== undefined ){
				this.$.attr( 'src', this.tablet_src );
				return;
			}
			else if( this.mobile_src !== undefined ){
				this.$.attr( 'src', this.mobile_src );
		...
		// Desktop
		this.$.attr( 'src', this.default_src );
	}

	// Indica o gadget pelo tamanho atual da tela
	window.gadgetBySize = function(){
	...
	jQuery.fn.reset = function() {
		return this.each( function(){
			...
			var $button = $( '<input type="reset" />' ).hide();
			var $form;
			...
			$( 'select' ).each(function(){
				var $select = $( this );
				var data_select2 = $select.data( 'select2' );
				if( data_select2 !== undefined ){
					data_select2.val( $select.val() );					
				}
			});
