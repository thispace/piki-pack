...
class Piki_List_Table extends WP_List_Table {
    ...
    function __construct( $options ) {
        ...
        # Primeira coluna
        $this->first_col = array_shift( array_keys( $options[ 'columns' ] ) );
    ...
    public function column_default( $item, $column_name ){
        ...
        if( $this->first_col == $column_name ):
            ...
            $actions = $this->_row_actions( $item );
    ...
    public function _row_actions( $item ){
        $actions = array();
        foreach ( $this->actions as $key => $action ):
            ...
            $tosprint .= '">'. $action .'</a>';
            $actions[ $key ] = sprintf( $tosprint, esc_attr( $_REQUEST[ 'page' ] ), $key, absint( $item[ 'ID' ] ) );
        endforeach;
        return $actions;
    }
    ...
    public function redirect( $url ){
        echo '<script type="text-javascript">window.location.href = "'. esc_url( $url ) .'"</sript>';
        exit;
    }
