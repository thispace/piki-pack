...
class PikiFlickr_Core {
	...
	function init( $options ) {
		...
		wp_enqueue_script( 'piki-flickr-gallery', plugins_url('/piki-flickr-gallery.js', __FILE__), array('jquery'), false, true );
		wp_enqueue_script( 'lightbox', plugins_url('/lightbox/jquery.lightbox-0.5.min.js', __FILE__), array('jquery'), false, true );
	...
	function stylesheets() {
		...
		elseif ( file_exists( TEMPLATEPATH . '/piki-flickr-gallery.css' ) ){
			...
			$css_file = plugins_url( 'piki-flickr-gallery.css', __FILE__ );
		}
		wp_enqueue_style( 'piki-flickr-gallery', $css_file, false, '2.70' );
		wp_enqueue_style( 'lightbox', plugins_url( '/lightbox/jquery.lightbox-0.5.css', __FILE__ ), false, '2.70' );
	}
	...
	function piki_flickr_ajaxurl() {
		echo('<script type="text/javascript">var PikiFlickr = { ajaxurl: "' . get_admin_url('admin-ajax.php') . '", pluginurl: "' . plugins_url( '/lightbox/', __FILE__ ) . '"}</script>');
	}
}