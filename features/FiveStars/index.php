...
class FiveStars {
    ...
    public static function add_files(){
        wp_enqueue_script( 'five-stars-scripts', plugins_url( '/scripts.js', __FILE__ ), array( 'jquery' ) );
        wp_enqueue_style( 'five-stars-styles', plugins_url( '/styles.css', __FILE__ ) );
    }
    ...
    public static function get_widget( $ID, $type="post" ){
        ...
        # HTML
        $widget = '<div class="five-stars-box clearfix" score-media="'. $score_media .'" user-score="' . $user_score . '" type="'. $type .'" rel="'. $ID .'">';
        for( $x=1; $x<=5; $x++ ):
            $widget .= '<span score="'. $x .'" class="star'.( $x <= $initial_scores ? ' active' : '' ).'">'. $x .'</span>';
        endfor;
        $widget .= '</div>';
