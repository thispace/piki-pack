...
class PikiYoutube {
	...
	function add_scripts() {
		...
		wp_enqueue_script( 'piki-youtube-gallery', plugins_url( '/piki-youtube-gallery.js', __FILE__ ), array( 'jquery' ), false, true );
		wp_enqueue_script( 'fancybox', plugins_url( '/fancybox/jquery.fancybox-1.3.4.js', __FILE__ ), array( 'jquery' ), false, true );

		wp_enqueue_style( 'piki-youtube-gallery', plugins_url( 'piki-youtube-gallery.css', __FILE__ ), false, '2.70' );
		wp_enqueue_style( 'fancybox', plugins_url( '/fancybox/jquery.fancybox-1.3.4.css', __FILE__ ), false, '2.70' );
	}

	function piki_youtube_ajaxurl() {
		echo('<script type="text/javascript">var PikiYoutube = { ajaxurl: "' . get_admin_url('admin-ajax.php') . '", pluginurl: "' . plugins_url( '/lightbox/', __FILE__ ) . '"}</script>');
	}
}
