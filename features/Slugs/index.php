...
Class Slugs {
    ...
    public function post_link( $post_link, $post, $leavename ){
        ...
        // Se o ID do preview foi passado na URL
        $preview_get = isset( $_GET[ 'preview_id' ] ) || isset( $_GET[ 'preview' ] ) ? true : false;
        $statues_out = array( 'auto-draft', 'draft', 'pending' );

        # Auto draft
        if( !is_admin() && ( is_preview() || $preview_get ) && in_array( $post->post_status, $statues_out ) ):
            return $post_link;
        endif;
    ...
    public function admin_footer(){
        ...
        echo '<script type="text/javascript" src="'. plugins_url( '/scripts.js' , __FILE__ ) . '"></script>';
    }
